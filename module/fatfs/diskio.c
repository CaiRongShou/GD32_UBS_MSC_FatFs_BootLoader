...
)
{
	...
	switch(pdrv) {
		case SD_CARD :
			...
			return res;
		case SPI_FLASH :
			SPI_FLASH_BufferRead((uint8_t *)buff, sector * FLASH_SECTOR_SIZE, count * FLASH_SECTOR_SIZE);
			res = RES_OK;
			return res;
		case INTER_FLASH:
//			btrd = (uint32_t *)buff;
...
)
{
	...
	switch(pdrv) {
		...
		case SPI_FLASH:
			...
			SPI_FLASH_BufferWrite((uint8_t *)buff, sector * FLASH_SECTOR_SIZE, count * FLASH_SECTOR_SIZE);
			res = RES_OK;
			return res;
		case INTER_FLASH:
//			fmc_unlock();
